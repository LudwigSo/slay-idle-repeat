...
public sealed class MigrationPlanTests
{
    ...
    public void The_shipped_history_is_exactly_the_files_this_build_carries_in_order()
    {
        ...
        shipped.Select(s => s.FileName).ShouldBe(new[]
        {
            ...
            // ⚠️ M5-08 authored this as 0006 over a base whose history ended at 0005. Renumbered to
            // 0008 at the integration merge, per the note above: 0006 and 0007 had landed meanwhile,
            // and a reserved 0009 would be the gap Ordered() refuses at boot.
            "0008_mail_segment_sends.sql",
        });
    ...
    {
        var declared = TableColumnsOf("0008_mail_segment_sends.sql", "mail_segment_sends")
        ...
        written.OrderBy(c => c, StringComparer.Ordinal).ShouldBe(
            declared.OrderBy(c => c, StringComparer.Ordinal),
            "0008_mail_segment_sends.sql and PostgresMailSegmentAudit.AppendStatement are the two "
