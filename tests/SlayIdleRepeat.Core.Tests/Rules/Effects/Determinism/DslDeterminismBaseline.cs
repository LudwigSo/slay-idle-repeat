...
/// <summary>
/// Reads <c>DslDeterminismBaseline.json</c> — M2-17's committed `18` §8 determinism baseline — out
/// of this assembly's embedded resources.
/// </summary>
/// <remarks>
/// <para>
/// 🔴 <b>Read <see cref="HeaderLines"/> before reading anything else.</b> This table is <b>not</b> a
/// reference-vector table in M0-06's or M0-07's sense and must not be described as one. Those were
/// generated by independent implementations and validated against externally published vectors —
/// xxHash64's and Landon Curt Noll's. There is no published authority for
/// <em>build permutation → hash</em>, so this table is necessarily self-generated, and what it
/// proves is <b>stability</b> and not correctness. Steering S5 cannot be satisfied here; saying so
/// is the alternative to appearing to satisfy it.
/// </para>
/// <para>
/// 🔒 <b>Nothing here re-derives a hash: it only parses.</b> That much it does share with
/// <c>CanonicalReferenceVectors</c> and <c>Hash64ReferenceVectors</c>.
/// </para>
/// <para>
/// ⚠️ <b>Three refusals, on <c>ContentValidator --write-baseline</c>'s design.</b>
/// <see cref="Validate"/> throws when the review status is not <c>reviewed</c>; when the review
/// block is incomplete — no <c>why</c>, <b>or no <c>reviewedOn</c>, or no <c>reviewedBy</c></b>; and
/// when any named row carries no <c>why</c>. A regenerated table nobody hand-edited therefore fails
/// on the very next run — which is the whole point, and is deliberate.
/// <c>DslDeterminismBaselineRefusalTests</c> drives all three and asserts which one fired.
/// </para>
/// </remarks>
internal static class DslDeterminismBaseline
{
    ...
    /// </summary>
    /// <remarks>
    /// <b>Separated from the static initialiser deliberately.</b> While these checks lived inline in
    /// the loader, the only thing that could exercise them was the committed file itself — which
    /// passes — so <c>The_committed_table_was_reviewed_by_a_human</c> and
    /// <c>Every_committed_row_says_what_it_pins</c> were asserting conditions the loader had already
    /// guaranteed: two tests that could not fail. The refusals are the whole reason a regenerated
    /// table cannot go green on its own, so they are the last thing that should be untested.
    /// <c>DslDeterminismBaselineRefusalTests</c> drives all three, and asserts <em>which</em> one
    /// fired.
    /// </remarks>
