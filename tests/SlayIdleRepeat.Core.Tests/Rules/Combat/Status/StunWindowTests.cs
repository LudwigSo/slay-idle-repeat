...
/// <summary><c>STUN</c>: max 1.5 s per application, with a mandatory 3 s immunity window after.</summary>
/// <remarks>
/// Immunity is mandatory — without it, stun-locking becomes the only viable build. Each half is
/// probed with the other satisfied, because either alone leaves that failure mode: the cap alone
/// permits a stun reapplied every 1.5 s forever, and the window alone permits one 30 s stun.
/// </remarks>
public sealed class StunWindowTests
{
    ...
    /// <summary>A 4 s application is clipped to the 1.5 s cap, which is thirty ticks.</summary>
    /// <remarks>
    ...
    /// <summary>A second stun inside the immunity window is refused outright, and the first one is not extended by it.</summary>
    /// <remarks>
    /// Both halves of the assertion are load-bearing. A refusal that still extended the live stun
    /// would be a stun-lock wearing a refusal's return value: the actor would never act again while
    /// the attacker kept applying. So the tick the actor recovers on is asserted to be unchanged by
    ...
    /// <summary>The window is measured from the tick the stun ends, not from the tick it was applied.</summary>
    /// <remarks>
    /// The discriminating case, and the reason this is its own test: measured from application, a
    /// 1.5 s stun applied at tick 0 would leave the actor stunnable again at tick 60; measured from
    /// the end, at tick 90. The difference is half the window's protection, and no test of a single
    /// stun would notice it, because a single stun behaves identically either way.
    /// </remarks>
    [Fact]
    public void The_immunity_window_runs_from_the_end_of_the_stun_and_not_from_its_application()
    {
        ...
        // Tick 61 is the only probe that separates the two readings. Application-anchored gives
    ...
    /// <summary>
    /// Clearing the stun leaves the immunity window standing — <c>REMOVE_STATUS</c> must not make a
    /// cleansed actor a better stun-lock target than an uncleansed one.
    /// </summary>
    [Fact]
    ...
    /// </summary>
    /// <remarks>
    /// The tick is 0.05 s. A window of 0.01 s rounds to zero ticks, which is no immunity at all —
    /// stun-locking — and would otherwise be indistinguishable from a correctly configured engine.
    /// </remarks>
    [Fact]
    ...
    /// <summary>The window is the whole protection: a stun reapplied on every tick still leaves the actor able to act for most of the fight.</summary>
    /// <remarks>
    /// The outcome that matters — "stun-locking becomes the only viable build" — rather than the
    /// two constants the tests above pin. Over 1800 ticks the pattern is 30 stunned then 60 free.
    /// An implementation that refused correctly but extended on refusal would score 0 here and pass
