...
public sealed class CommandVocabularyTests
{
    ...
    public void Every_command_in_the_vocabulary_is_applied_and_refused_rather_than_thrown()
    {
        ...
        // BEGIN_SESSION — M3-15 lowered it again to 47 by START_RUN, and M3-04 lowers it to 45 by
        // ROLL_DICE and USE_REROLL. It is stated as "the registry minus the handled rows" rather than
        // as the literal 45 so the next task to land a handler lowers it by construction, and so the
        ...
        deferred.ShouldBe(
            ...
            "every DEFERRED row of 14 §2.3 is driven here — 45 of the 49 since M3-04 landed the " +
            "ROLL_DICE and USE_REROLL handlers. A mismatch means the loop skipped a deferred row " +
        ...
        deferred.ShouldBe(
            ...
            "is 49 rows and exactly four of them — BEGIN_SESSION (30 §2.3's day cycle), START_RUN " +
            "(02 §2's runSeed commit), ROLL_DICE and USE_REROLL (04 §§1,3-4) — have a handler. Lower " +
            "this by exactly the number of rows that become Handled, and never to a number the loop " +
            "cannot reach.");
    }
