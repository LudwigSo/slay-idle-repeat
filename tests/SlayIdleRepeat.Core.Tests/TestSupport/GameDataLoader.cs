...
/// <summary>
/// This suite's <c>game-data</c> → <see cref="ContentSnapshot"/> loader — the one place in
/// <c>SlayIdleRepeat.Core.Tests</c> that knows JSON exists.
/// </summary>
/// <remarks>
/// The suite reaches <c>SlayIdleRepeat.Core</c> and nothing else, so it cannot reach
...
internal static class GameDataLoader
{
    ...
    public static string FindRepositoryRoot()
    {
        ...
        return directory?.FullName
            ?? throw new InvalidOperationException(
                ...
                "This suite reads the authored game-data out of the checkout it is running " +
    ...
    private static SortedDictionary<string, string> ReadDocuments(string dataRoot)
    {
        ...
        if (!Directory.Exists(dataRoot))
        {
            throw new DirectoryNotFoundException(
                $"'{dataRoot}' does not exist. This suite reads the authored content out of " +
    ...
    /// test runs from each other, not content builds.
    /// </remarks>
