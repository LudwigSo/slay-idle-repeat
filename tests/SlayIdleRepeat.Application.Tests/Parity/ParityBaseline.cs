...
/// <summary>One individually pinned sequence, as the committed table holds it.</summary>
/// <param name="Id">The row id.</param>
/// <param name="Sequence">The sequence ordinal the row's property first holds at.</param>
/// <param name="Steps">How many commands that sequence carries.</param>
/// <param name="Hash">The wire hash of everything both hosts answered for it.</param>
/// <param name="Why">Which property of the corpus this row defends. Hand-written, never generated.</param>
internal sealed record ParityRow(string Id, int Sequence, int Steps, string Hash, string Why);

/// <summary>
/// Reads <c>ParityCorpusBaseline.json</c> — `14` §13's committed parity corpus — out of this
...
/// with no <c>why</c>.
/// </remarks>
internal static class ParityBaseline
{
    private const string ResourceName =
        "SlayIdleRepeat.Application.Tests.Parity.ParityCorpusBaseline.json";

    /// <summary>The committed file's text, exactly as it is committed.</summary>
    ...
    /// <summary>The sequence count the committed table was built over.</summary>
    internal static int Sequences { get; } = Table.GetProperty("sequences").GetInt32();
    ...
    /// <summary>The wire hash of the state every sequence starts from.</summary>
    internal static string BaselineState { get; } = Table.GetProperty("baselineState").GetString()!;

    /// <summary>The one hash over all the chunks.</summary>
    internal static string Aggregate { get; } = Table.GetProperty("aggregate").GetString()!;

    /// <summary>The chunk hashes, in ordinal order.</summary>
    ...
    internal static IReadOnlyList<ParityRow> Named { get; } = RowsIn(Document);
    ...
    /// <summary>Which task's review this is.</summary>
    ...
    internal static ParityRow Row(string id) =>
    ...
    {
        using var stream = typeof(ParityBaseline).Assembly.GetManifestResourceStream(ResourceName)
            ?? throw new InvalidOperationException(
                $"The parity baseline '{ResourceName}' is not embedded in " +
                $"{typeof(ParityBaseline).Assembly.GetName().Name}. Available: " +
                string.Join(", ", typeof(ParityBaseline).Assembly.GetManifestResourceNames()));
    ...
    internal static JsonDocument Validate(JsonDocument document)
    {
        ...
        if (!string.Equals(status, ParityBaselineWriter.ReviewedStatus, StringComparison.Ordinal))
        {
            throw new FormatException(
                $"ParityCorpusBaseline.json is still '{status}'. The regeneration command writes the " +
                "SHAPE — 1 000 sequences, their chunk hashes and one aggregate — and stops there. " +
                "Which part of either dispatch pipeline moved, and whether it was meant to, is " +
                "written by hand. An unreviewed parity break is a parity break that was accepted " +
                "without anybody saying so. Read the diff, write review.why, then set review.status " +
                $"to '{ParityBaselineWriter.ReviewedStatus}'.");
        }
        ...
        {
            throw new FormatException(
                "ParityCorpusBaseline.json says it was reviewed and does not say who concluded what, " +
                "or when. review.why names the change that moved the hashes and the task that made " +
                "it, review.reviewedBy names the task and review.reviewedOn the date; without all " +
                "three the 'reviewed' status is a checkbox rather than a record.");
        }
        ...
        {
            throw new FormatException(
                "These named rows of ParityCorpusBaseline.json carry no 'why': " +
                $"{string.Join(", ", unexplained)}. A named row exists to say which property of the " +
                "corpus it defends — 'the first sequence that leaves its run mid-walk', not " +
                "'sequence 7'. A row that cannot say what it pins pins nothing a failure message can " +
                "use.");
        }
