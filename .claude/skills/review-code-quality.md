...
2. Read the relevant sections of `game-design/14_TECHNICAL_ARCHITECTURE.md` and `game-design/23_PORTS_AND_ADAPTERS.md` first (this repo has no separate `CONVENTIONS.md`/`ARCHITECTURE.md` yet), plus `game-design/18_EFFECT_DSL.md` if the change touches perks/talents/gear/bosses, so you don't re-derive conventions from scratch. Then read each target file fully, **plus the immediate types it calls into** — docs-first cuts exploration, but never judge a changed call without reading its direct collaborators. Explore wider only when the docs are silent.
...
### Types & data
- Prefer `readonly` fields and immutable `record`/`record struct` for value-like data — this project already leans on this for aggregates like `CombatEvent`, `SimulationResult`, `DieFace`, `AdOutcome`; match that pattern for new value types.
...
- Domain/rule invariant violations should throw a specific, purpose-built exception type — not a bare `Exception`/`ArgumentException` — so callers (an endpoint's exception handler, a use case's caller) can map it deliberately rather than by string-matching a message.
...
- Command handling follows the project's idempotency protocol: a command's `commandId` is checked against stored processed-command state before re-executing, and the stored outcome is replayed for a duplicate rather than the command running twice.
