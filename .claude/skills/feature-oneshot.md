...
src/SlayIdleRepeat.Core/            Pure rules. Zero dependencies — not even a clock. Fully synchronous.
                                     ...
                                     Handlers/ GameRules.cs Testing/   (30 §11 — GameRules.Apply is the only
...
src/SlayIdleRepeat.Contracts/       Wire envelopes only (commandId, sequence, stateHash, error shapes) —
                                     never re-declares a command, event, or domain type (30 §11.6)
...
game-data/                shared JSON content, embedded in client + server — tunables live in
                                     tuning/*.json specifically, schemas in schema/ (21 §3.1)
...
- **Comments are the exception, not the default — and never a permanent citation to `game-design/`.** Well-named code and tests should read on their own; add a comment only when it captures a genuinely non-obvious constraint (a hidden invariant, a workaround for a specific bug, a concurrency/ordering rule, tricky math) that the surrounding code can't express by itself. As a rough ceiling, comment lines should stay near **10%** of a file's lines — new code much denser than that is itself a Phase 4/5 finding to fix, not wave through. Never cite a `game-design/NN_*.md` section number or doc title inside a comment: those docs are scaffolding for getting the initial build right, not a permanent part of this codebase, and a comment that leans on one as its explanation reads as broken the day the doc is archived. When a design doc is the *reason* for a rule, pull the reason into the code itself — a well-named constant, an assertion message, a test case name — instead of pointing at the doc.
...
Review the structural/boundary impact (diff vs `<BASE_REF>`). The first question is always **the engine boundary**: did any game rule leak into a `Node`, or any Godot/vendor type leak into `Core`/`Application`? Then: every new port has its in-memory fake, no concrete adapter is named outside a composition root, no `if (isSubscriber)` branch appeared anywhere else, no hardcoded per-perk/per-boss special case bypassed the Effect DSL (sanctioned exception: `MODIFY_DIE_FACE`, 16 §A7 ruling 9), every new mutation goes through `GameRules.Apply`, every randomised grant routes through `LuckService` (24 §11), every currency mutation emits `CurrencyChanged`, and new balance numbers landed in `game-data/tuning/*.json` rather than in code. Apply a fix for every finding, keep tests green, commit. Loop to Phase 1/3 automatically if a finding demands it. → If the branch touched scenes, theme resources, or UI scripts under `res://game/`, proceed to Phase 6; otherwise skip Phases 6–8 and go straight to Phase 9.
