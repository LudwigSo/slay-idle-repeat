...
- New balance/tunable numbers are added to `game-data/tuning/*.json`, not hardcoded as C# constants inside `Core`/`Application` — and runtime-delivered content (event packages, feature flags) arrives via the content endpoint / `IRemoteConfigPort` into `GameContext`, not by editing shipped files (26 §8).
