...
- **Dispatched agents inherit `feature-oneshot`'s comment discipline.** Every dispatched task runs `feature-oneshot`, which already keeps comments sparse (~10% of a file's lines, only for a genuinely non-obvious *why*) and forbids citing a `game-design/NN_*.md` section inside a comment — those docs are initial-build scaffolding, not a permanent reference. When pasting design-doc excerpts into a task prompt, paste them as context for the agent to build correct behaviour from, never as text to be echoed into a code comment or doc citation.
