...
Design-doc authority: `docs/ARCHITECTURE.md` governs `Core`'s shape (the `GameRules.Apply` seam, rejection-not-exception, `InternalsVisibleTo` for `Core.Tests` only) and the layer boundaries; `docs/game-design.md` carries the design intent behind them. Where both are silent, the enforcing tests under `tests/SlayIdleRepeat.Architecture.Tests` are the next authority — read them rather than inventing a convention.

🔒 **This repository has no integration or end-to-end test tier, and you must not create one.** `SlayIdleRepeat.Integration.Tests` was deleted deliberately. Never scaffold a replacement under any name (`*.E2E.Tests`, `*.System.Tests`, `*.Smoke.Tests`, an "integration" trait bucket inside a unit suite), never add a test project that boots Docker, a live Postgres/Redis/MinIO, a real ASP.NET host, a real vendor SDK or the Godot runtime, and never start infrastructure yourself. **`SlayIdleRepeat.Contract.Tests` (each port's suite run against every real implementation) does exist, but is not part of this workflow either.** Unit tests against `SlayIdleRepeat.Core` and `SlayIdleRepeat.Application` (the latter run against `SlayIdleRepeat.Adapters.InMemory`, never a real adapter) are the centerpiece and the only tier you write here — never propose, scaffold, or extend an integration or end-to-end test as part of this workflow, even if the feature has an HTTP surface or touches persistence. An HTTP endpoint or a repository call is still tested at the unit tier: the use case against an in-memory fake, exactly like every other use case.
...
1. **Clarify before writing anything** — This step is **mandatory and always runs**, even if the request looks complete. You must ask every open question you have before writing a single line of test code. Do not skip this step because the request seems clear enough.
   ...
   - **Invalid / error cases** — What should the system do when input is invalid or a precondition is not met (e.g. drafting a perk not in the offered pool, merging items of different rarities, rolling with zero reroll charges)? For a `Core` command the shape of the answer is fixed — a `CommandResult` with `Accepted == false` and a `RejectionReason`; domain rules never throw — so the real questions are *which* rejection reason, and what the caller/UI does with it.
   ...
   - **Determinism** — Does this touch the deterministic draw streams (counter-based: `Hash64(runSeed, streamName, drawIndex)`), the combat simulator, or board generation? If so, which stream (`board`, `dice`, `draft`, `drops`, `combat:{battleIndex}`, `events`, `minigame:{index}`), and is a fixed-seed byte-identical outcome part of "done"?
   - **Luck protection** — Does this grant a randomised item (gear, pet, mount, draft option)? Every such grant routes through `LuckService` with a declared source class (the schema validator fails a grant source with no class). Does it read or advance a pity counter, and at which scope (player profile; per-gear-instance `ENHANCE`, inherited by merge outputs; per-run `DRAFT`)? A pity guarantee needs an exact-`N` unit test plus a 100,000-sequence property test.
   ...
   - **Which suite(s)** — See the routing table below. There is no integration/E2E suite in this repository at all — everything routes to a unit-level suite.
...
3. **Write the tests** — Route each test to the correct suite and follow [unit-testing](unit-testing.md) for conventions:
   ...
   Never add anything to `SlayIdleRepeat.Contract.Tests` from this workflow, and never create an integration/E2E suite to hold it, no matter how cross-cutting the feature feels.

4. **Record the coverage map** — Before handing off, output a table mapping **each acceptance criterion (or numbered requirement) → the test(s) that pin it, and at which tier**. Name any criterion you deliberately did not cover and why (a legitimate reason here is "requires a real adapter — belongs to `SlayIdleRepeat.Contract.Tests`, out of scope for this workflow" or "cannot be verified without live infrastructure, so it stays an open gap" — say so explicitly rather than silently dropping it, and never resolve it by proposing an integration suite). This is the artifact that makes the final verification honest. If the workflow keeps a handover document, the map belongs in it.
...
- Assert on public state, return values, `CommandResult`s and emitted `DomainEvent`s, `RejectionReason`s (domain rules reject, they never throw), persisted state (through the in-memory fake, at the Application seam), `SimulationResult`/`LogHash`, and other side-effects visible through a port.
...
- The default test has **zero comments** — the name states the case, the assertions state the contract, the `because` string states why the number is that number. The same review cut ~3,000 comment lines, almost all: `<summary>` tags restating the test name, remarks duplicating the `because` string, project history ("added after review found…" — that's `git log`'s job), banner dividers, and essays defending a test's existence.
...
- Add a test to `SlayIdleRepeat.Contract.Tests`, create an integration/E2E suite under any name, or write a test that requires Docker, a real database, a real ad SDK, or booting the Godot runtime.
- Start infrastructure of any kind — `docker compose up`, `docker run`, a local Postgres/Redis/MinIO, or the real server — for any reason, including "just to see whether the test would pass".
