...
You review the **player experience** of the changed screens/flows (see `game-design/13_UI_UX_SCREENS.md` §1 for the full S01–S38 screen list — including the second-pass S28–S29 Dungeons, S30–S32 Events, S33–S36 Guilds, S37 Inbox, S38 Feats — and this project's own UX principles). In scope:
...
- The primary action sits in the bottom-third, one-thumb-reachable zone — this project states outright that the Roll button must be "the largest interactive element on screen" in that zone (13 §3); any new primary action on a core-loop screen should be held to the same bar.
...
- **"Two taps from launch to rolling a die" is a stated hard target for the core loop** (`game-design/02_CORE_LOOP_AND_RUN.md`) — any change that adds a tap to reach the Board/Roll from Home on a returning player's path is a finding worth escalating even if it's Warning-severity elsewhere.
...
- The death/revive flow (S13) and connection handling never let a player lose progress they'd reasonably expect to keep — run state lives on the server for 48 h and resumes exactly (02 §1, 13 §11), a Stage-1 death still pays out (02 §5.2, "Death must still pay"), and the "Nothing is lost. Ever." promise (12 §2.2 — it is specifically about a lapsed Plus subscription) must hold in any Plus-adjacent flow; a flow that reads as risking loss even when it technically doesn't is still a UX finding (perceived safety matters as much as actual safety here).
...
- **Interstitial** placement never fires at a protected moment: mid-battle, mid-draft, after a death, during a reconnect, while a rewarded reward is being granted, during FTUE, or in the first 72 h post-install (12 §6). **Rewarded** placements are opt-in offers that deliberately exist at some of those exact moments (`AD_REVIVE` at 0 HP, `AD_REROLL_PERK`/`AD_EXTRA_PERK_CHOICE` on the draft screen) — judge them by opt-in clarity and the fairness contract, not the interstitial exclusion list.
- No placement may advance, reset, or protect a pity counter (12 §4.2), and on ad no-fill/error the reward is granted anyway and the cap slot consumed, server-side (12 §4.3) — UI implying otherwise is a finding.
