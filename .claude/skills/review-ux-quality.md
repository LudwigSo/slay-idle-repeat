...
You review the **player experience** of the changed screens/flows (see `game-design/13_UI_UX_SCREENS.md` for the full S01–S27 screen list and this project's own UX principles). In scope:
...
- The offline/revive/death flow (S13) never lets a player lose progress they'd reasonably expect to keep — this project's design explicitly promises "nothing lost on lapse ever"; a flow that reads as risking loss even when it technically doesn't is still a UX finding (perceived safety matters as much as actual safety here).
...
- Interstitial/rewarded-ad placement doesn't interrupt a moment the project explicitly protects (mid-battle, mid-draft, right after death, within the first 72h post-install, during FTUE) — a new ad trigger point should be checked against these.
