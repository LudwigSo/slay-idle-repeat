...
3. Create the review branch: `review/M<N>` off the milestone head. All fixes this skill applies land here — never on the base branch, never on `milestone/M<N>` itself.
   🔒 **Exception, and check for it before branching: if a LATER milestone was kicked off on top of this one and has already merged, branch off the current integration point instead** (`main`, usually), and say so plainly in the report. M4 hit this — M7 was based on `milestone/M4` and had already fixed two of the defects M4's exit criteria were blocked on, so a branch cut from the milestone head would have fought M7 at merge and re-derived work already done. **The reviewed diff range does not change** — only where the fixes land. Verify with `git branch --contains` and `git merge-base --is-ancestor` rather than assuming.
   ⚠️ Also verify the milestone branch pointer actually *is* the milestone head: M4's `milestone/M4` sat one commit past it, on the *next* milestone's kickoff commit.
...
**Fix policy: auto-apply, same as `feature-oneshot`.** Apply a focused fix for every finding, keep **every** suite green after each dimension — run the CI scripts (`pwsh build/ci/Invoke-UnitTests.ps1 -Group unit` and `-Group architecture`) rather than a hand-picked subset, because the suite set grows and a stale list silently reviews less than it claims — and commit per dimension (`Review M<N>: <dimension> fixes`).

🔒 **If you delegate fixes to more than one agent, give each its own worktree. A file-territory split is not enough.** A review pass runs S1 mutations, and an S1 probe is a temporary edit to *production* code: in M4 two fix agents on one checkout leaked probes into each other's exclusive files twice, one of which forced a guaranteed reward on every draw and was announced to the sibling as an intentional change not to revert. **And a green suite cannot detect it** — a probe reverted and re-applied between runs leaves no trace in a test result. After any shared-checkout episode, read the production diff line by line and audit BOM/line endings (`text=auto` normalises endings, not byte-order marks). Prefer sequential passes over a shared checkout. Findings that are too large to fix safely here (a redesign, a cross-milestone refactor) are not silently dropped: record them and carry them to the retro (Phase 3) and, if they warrant tracked work, add them to the tracker as new task rows or flag them for the next milestone's kickoff.
