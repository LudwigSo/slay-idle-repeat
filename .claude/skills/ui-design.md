...
4. **Check responsiveness** across at least the narrow and wide ends of the supported portrait range (9:16 and 9:20, base canvas around 1080×1920 up to 1080×2340) and against safe-area padding, before calling a view done, even if the user didn't ask — a layout that only works at one aspect ratio isn't finished, and this project explicitly supports a range.
5. **Stay within the established visual language** unless the user is deliberately changing it: rounded 24dp panel corners with a 3dp dark outline and soft drop shadow; chunky high-contrast buttons with a 4dp pressed-state offset; the fixed rarity colour palette (C `#9AA5B1` · B `#4CAF50` · A `#3B82F6` · S `#F5A623` · SS `#C13BE8`); numbers abbreviated above 10k with full value on long-press; PvP tier/Plus shown as a text label in tier colour, never a badge/frame asset (none exist in v1).
