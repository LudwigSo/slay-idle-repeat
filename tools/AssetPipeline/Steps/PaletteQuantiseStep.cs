...
/// <summary>Palette quantise: snaps to the biome palette + neutrals (biome assets only).</summary>
/// <remarks>
/// <para>
/// Biome assets only: a row with <see cref="AssetSpec.IsBiomeScoped"/> false comes back
...
/// </para>
/// <para>
/// Where it does apply: a nearest-colour snap to the biome's six locked hues plus the outline
/// colour plus <see cref="ThresholdKeys.PaletteNeutrals"/>, within
/// <see cref="ThresholdKeys.PaletteMatchTolerance"/>. Both of those are uncalibrated, so both throw
/// when unstated.
/// </para>
/// <para>
/// A pixel further from every authorised colour than the tolerance is left exactly as it is: the
/// tolerance separates "antialiasing of an on-palette colour" from "a colour that is not on the
/// palette at all", and snapping the second kind anyway would quietly repaint art the palette does
/// not cover.
/// </para>
/// </remarks>
public sealed class PaletteQuantiseStep : IAssetStep
{
    ...
    /// <summary>
    /// The biome's six locked hues, the outline colour, and the unenumerated neutrals the caller
    /// has stated.
    /// </summary>
    ...
    /// <param name="authorised">The closed set of colours this step may produce.</param>
