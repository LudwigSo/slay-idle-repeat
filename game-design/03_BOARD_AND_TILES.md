...
| `TILE_CAMPFIRE` | Campfire | 🔥 | Guaranteed 1 before boss | Choose: heal 40% Max HP · upgrade one owned perk to its next tier · **gain one fixed die** (`04` §6). 🔒 Three options again. The third seat held *gain 2 Reroll Charges*, was emptied with the reroll (`04` §5), and now grants a fixed-die choice — the player names the number, so the campfire spends itself on a choice rather than on a die. |
...
| `CON_FIXED_DIE_TOKEN` | Fixed Die Token | **Instant: +1 fixed die** (`04` §6) | Never held. The player names the die's number after the purchase, so the token is spent on a choice rather than on a number the shop picked. Uncapped, so it never greys out. |
...
- **Held cap: 4** 📐, counted across all held consumables (Draughts + Ropes). Only Draughts and Ropes are ever actually held — the three tokens convert to their grant at the till. ⚠️ Fixed dice themselves are **uncapped** and counted separately (`04` §6.3); this cap is about pouch slots, and a token never occupies one. A purchase or event grant that would exceed the cap is unavailable (the shop slot greys out); an event grant clamps to what fits (`EVT_SPRING`'s ×2 grants one if only one fits).
