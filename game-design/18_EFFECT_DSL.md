...
| `RANDOM_OUTCOME` | Draw **one** value from the battle's combat stream over the `outcomes` weight table and fire the single effect that row names — the **mutually exclusive** choice §10.1 E6 adds for the Dicelord's *Roll of Fate* (`17` §9). Each row is `{"effectId": …, "weight": …}` and references an effect **by id**, never embedding one; weights are relative, and a `0` weight disables that row. Carries **no** `value`: its own number is the 1-based index of the row that won. 🔒 Exactly one draw index per roll — three `chance`-gated effects would be three *independent* draws, which is neither mutual exclusion nor one d6 |
