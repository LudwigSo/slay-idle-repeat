...
- **Within one attack resolution**, each side's stats re-aggregate against the other party: the attacker's block with the defender as current target, the defender's with the attacker in context. A gated standing effect therefore contributes against exactly the targets its gate names — `+X% Damage vs Elites` composes ×(1 + v) into step 5 only when the swing's target is an elite, and Ironvow's `−15% damage taken from Elites/Bosses` multiplies `DR%` below 1.00 only for a hit from an elite or boss attacker. The per-pair block lives for that one resolution and is never stored.
