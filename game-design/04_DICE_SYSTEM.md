...
🔒 **The whole board is drawn at all times.** There is no fog, no preview range and no reveal distance: every tile of every stage exists from the moment the run starts. This is load-bearing rather than a convenience — a die that only answers a number is only an interesting decision if the player can see what the numbers reach, and it is the entire reason a *fixed* die is worth choosing a number for. ⚠️ `16` **D67** amends what "visible" means and nothing about what is drawn: the board is a 3D scene the camera rides through, so it is **reachable** at all times through the three-state control of `03` §8 rather than framed whole at all times. The reason the amendment does not cost this section its argument is that its middle state frames the current stage at a size the tiles can be read at — which framing the whole of a long board cannot do.
