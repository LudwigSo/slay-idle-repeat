...
- 🔒 **The whole track is drawn, always** — no fog, no preview range, nothing clipped (`16` D42). It is the reason a *fixed* die is worth choosing a number for: the player is reading what the numbers reach.
- ⚠️ **`16` D67 amends the framing and not the drawing.** The board is a 3D scene the camera rides with the hero, so it is **reachable** at all times rather than framed at all times, and the wrapping-row clause is superseded — a chapter may author a board whose every tile, framed whole, falls under `03` §8's mandatory 48 dp. The three-state recenter control (`03` §8) is what reads the far board, and it sits in the track's own frame rather than in the thumb zone: it is a statement about the board, and the action column stays the run's.
