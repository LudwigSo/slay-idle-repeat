...
| **2 — Loaded Dice** | 66–33% | `AURA` — Roll of Fate now rolls **only 1–3** (never in the hero's favour on 3–4... see ruling below). `PERIODIC 14s` — **Scramble**: one of the hero's six die faces is replaced by `Void` for the rest of the fight. Purely cosmetic during combat — but it **persists into the remainder of the run** if the player survives, which makes killing him fast genuinely valuable. |
