...
✅ **The mechanical QA layer implements the *old* Part F, and says so.** `tools/AssetPipeline` carries nine `IQaCheck` implementations built against Part F's previous eleven lines. Those lines now live in `Qa/Doc15PartF.cs` as `SupersededItems`, beside `CurrentItems` — the twenty-six above, generated from this document and reconciled against it character for character by `QaChecklistTests`. **Exactly two of the eleven survive verbatim into the twenty-six**, and that number is asserted, so neither a reworded line here nor a re-pointed check can move it quietly. The nine checks measure **pixels**, which still answers for kinds **R** and **F** and answers nothing for **M**: the geometry, texturing and rig groups above have no implementation at all. Building one is its own task — see `16` D60 consequence 5.
