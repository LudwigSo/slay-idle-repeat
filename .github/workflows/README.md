...
| `Invoke-ContentValidation.ps1` — correct failure on today's empty tree; pass, bad-parse, duplicate-key, both orphan directions and the `schema-map.json` override all proven on fixtures | Runner-label availability (`ubuntu-24.04`, `macos-14`, `macos-15`) |
