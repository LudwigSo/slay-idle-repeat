...
| M1-04 | `Player` aggregate — **rescoped at wave 4** to the state whose element types exist: profile, the 7 meta currencies, `EnergyBanks`, FTUE progress, the daily/weekly counter *mechanism*; public getters / internal ctors; `PlayerSnapshot` + `Rehydrate`. 🔒 **Inventory, the unopened-container shelf, pity-counter storage and lifetime feat counters are deferred to GapRegister entries** — they need `GearInstance` (M4-03), container types (M4-02), `LuckService`'s pity-key vocabulary (M4-01) and the `counters.json` semantics that are **O29, undecided until the M16 kickoff**. Storing them now means inventing those sets (S6). Also **not** entitlement — see the wave-4 ruling | 30 §4, §11 | ⬜ |
