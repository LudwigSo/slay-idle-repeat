...
| ID | Task | Spec | Status |
|---|---|---|---|
| M0-01 | Solution skeleton per the authoritative layout: `Core`, `Contracts`, `Application`, `adapters/{client,server,fakes}`, `Server`, `Client` (placeholder), `SlayIdleRepeat.Data`, `tools/{BalanceHarness,EconomySim}` (empty), `tests/` (5 projects) | 23 §3 | ⬜ |
...
| M0-10 | `SlayIdleRepeat.Data` initial layout: `schema/`, `tuning/` (the 14-file skeleton + `sim_thresholds.json`), `loc/en.json` + `de.json`, `content/` | 21 §3.1 | ⬜ |
