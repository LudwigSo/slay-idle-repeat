...
| M1-03 | Public `DomainEvent` hierarchy + the `CurrencyChanged`-with-reason invariant (IL-scan test) | 30 §7 | 🔄 dispatched wave 3 · `feature-M1-03-domain-events` |
...
| M1-10 | Energy + Energy Reserve math: max/regen/costs, overflow routing, automatic spend order. Pure static functions over primitives in `Rules/Economy/` — `Model` never references `Rules` (`30` §11.5), so handlers call these and write the result | 10 §3, 28 C | 🔄 dispatched wave 3 · `feature-M1-10-energy-math` |
