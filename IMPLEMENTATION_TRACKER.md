...
| M1-02 | Public `GameCommand` hierarchy — **all 49 commands (19 run, 30 meta)** as the one wire+domain vocabulary, each registered in M1-06's dispatch table in the same change (the live `Every_command_type_is_handled_by_Apply` rule goes red otherwise) | 14 §2.3 | 🔄 dispatched wave 7 · `feature-M1-02-command-vocabulary` |
