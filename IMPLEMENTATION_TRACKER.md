...
| M2-11 | Enemy stat derivation + `EnemyLevel(c,t)`; 8 archetypes with coefficient rows; WARDEN/CASTER on-hit tables; elite system (×2.2, 8 modifiers, no-repeat rule); **`game-data/content/enemies/`** + schema, with 16 elite identities. Closes 5 📐-baseline entries | 05 §6 | 🔄 wave 4 · `feature-M2-11-enemy-derivation` |
