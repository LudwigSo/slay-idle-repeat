...
|---|---|---|---|
| M0 | Foundations, CI & week-1 spikes | pre-1, 2 (partial), spikes O14/O23 | 🔄 **implementation complete, awaiting review** — all 11 tasks merged to `milestone/M0` |
...
| ID | Task | Spec | Status |
|---|---|---|---|
| X-01 | **Determinism gate** — `Hash64` reference vectors, 4-dp rounding at every accumulation point, cross-platform `LogHash` CI (Linux x64 / Android ARM64 / iOS ARM64) on every commit | 14 §8, 05 §1.1 | 🔄 M0: `Hash64` + `DeterministicRng` + a 75-row vector table (M0-06) and `CanonicalStateWriter` + a 29-row table (M0-07), both validated against **externally published** vectors, not self-generated ones. **Remaining:** 4-dp rounding lands with combat (M2); the cross-platform `LogHash` CI is M5-12 — note `macos-14` is unsupported from 2026-11-02 |
| X-02 | **Architecture test suite** — dependency rule, port purity, banned ambient APIs, `Apply`-only mutation, playable-from-Core-alone, internals, entitlement/guild isolation; fails the build | 23 §6, 30 §9 | 🔄 M0: **33 live rules, 0 skipped**, failing the build, proven to bite by six red-then-green demos. 11 are vacuous until M1 populates their subject sets — by design, never `[Skip]`; a meta-rule fails the build on any skipped rule |
| X-03 | **Content schema validation + tuning discipline** — build fails on unknown IDs/orphans/duplicates; every 📐 number lives in `SlayIdleRepeat.Data/tuning/` (14 files), enforced by the marker-vs-schema build check; experiments as sparse override patches only | 14 §6, 21 §3 | 🔄 M0: all five `14` §6 failure classes have committed negative tests; the 📐 audit runs in **3 directions** against a dated 46-entry baseline that can only shrink; overrides are sparse patches that never touch canonical files. ⚠️ `tuning/` is **16** files per `21` §3.1 — `14` §6's "14 files" is stale prose |
...
| M0-10 | `SlayIdleRepeat.Data` initial layout: `schema/`, `tuning/` (**the 16 files catalogued in `21` §3.1**, incl. `sim_thresholds.json` — `14` §6's "14 files" is stale prose and defers to 21), `loc/en.json` + `de.json`, `content/` | 21 §3.1 | 🔍 merged · `feature-M0-10-data-layout` — 16 tuning files (set-equality verified), 19 draft-2020-12 schemas, 0 orphans either direction, 37 JSON files all parse. **No number invented**: 98 unauthorised leaves are `null` by documented convention, so every hole is greppable. 82 loc keys, DE entirely `##TODO_DE##` (D20: nothing machine-translated) |
...
| M7-10 | Client CI: Android debug APK through the custom export template (with the MAX plugin present but stubbed), iOS export smoke build | 14 §14 | 🔍 merged · `feature-M0-02-ci-pipeline` — 9 CI jobs + nightly, 5 PowerShell gate scripts, server Dockerfile (non-root, 12-factor, digest-pinned). 6 jobs live, 2 gated (determinism → M5-12, android-export → M7-10). ⚠️ **Authored, never observed green — no git remote exists** |
