...
| M1-12 | **Verify the ten Core architecture rules woke up** now that M1 populated their subject sets, and delete the now-false "vacuous until M1" comments. The rules themselves already exist and are live from M0-08 — this task adds no new rule, it proves each one now bites on real subjects | 30 §9 | 🔄 dispatched wave 11 · `feature-M1-12-architecture-verification` |
