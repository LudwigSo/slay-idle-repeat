...
| M2 | Effect DSL & combat simulation | 1 | 🔄 **carry-forwards R1–R4 implemented on `review/M2`, awaiting human sign-off** — 17/17 tasks merged, milestone-reviewed, and all four carry-forwards (below) now applied. Clean build 0 warnings/0 errors · Core **3062** · Application **555** · Contract **24** · Architecture **79/79** · content validation OK · 📐 baseline unchanged **38**. **Exit criteria: ① still met-with-caveat (see M2-R2, now closed — `ON_LETHAL` fires; caveat downgraded to informational), ② met, ③ met for the engine given a hero that can act (see M2-R3 — one new, separate Cogitator Prime fault found under real hero construction, tracked as a fresh open item below, not one of R1–R4), ④ met.** `05` §9's guardrails 1/5/6 still fail and 3/4 are still largely unmeasurable at par — tuning decisions on `game-data/`, not code; R1 makes bosses genuinely harder (fired stat ops, incl. `SYS_ENRAGE`, now apply) so these numbers will move again once tuned. ⚠️ **A second, independent session worked this same handover in parallel** on worktree `kickoff-milestone-m8-669804`, branch `fix/M2-carry-forwards` — it took the *other* defensible option for R3's `CURRENT_TARGET` ambiguity (data retarget vs this track's engine change) and its R4 attempt non-compliantly widened `Domain.PublicRuleTypes` from 6 to 11. Left untouched at the user's instruction; needs human reconciliation before either can be treated as canonical. **Merges to `main` only after M1 is merged and verified.** Next: human review of `review/M2`, reconcile with `fix/M2-carry-forwards`, then sign off |
...
| X-02 | **Architecture test suite** — dependency rule, port purity, banned ambient APIs, `Apply`-only mutation, playable-from-Core-alone, internals, entitlement/guild isolation; fails the build | 23 §6, 30 §9 | 🔄 M0: **38 live rules, 0 skipped** (33 at merge, +5 in review), failing the build, proven to bite by six red-then-green demos. 11 are vacuous until M1 populates their subject sets — by design, never `[Skip]`; a meta-rule fails the build on any skipped rule |
