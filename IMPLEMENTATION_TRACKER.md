...
| M2 | Effect DSL & combat simulation | 1 | 🔄 **implementation complete, reviewed, NOT signed off** — 17/17 tasks merged; milestone review complete on `review/M2` (4/4 applicable passes; UI+UX N/A, no presentation surface). Clean build 0 warnings/0 errors · Core **3015** · Application **555** · Contract **24** · Architecture **79/79** · content validation OK · 📐 baseline **46 → 38**. **Exit criteria: 2 met, 1 met-with-caveat, 1 NOT met.** ① 44 ops/23 triggers/23 conditions unit-tested — **met with caveat**: all are *tested*, but `ON_LETHAL` is fired by nothing in the engine and `ON_REVIVE` was unreachable until the review fixed it; *unit-tested ≠ wired*. ② `< 5 ms` — **met**, median **2.881 ms** on real content (p90 3.681). ③ all 8 bosses run from DSL data with zero bespoke code — 🔴 **NOT met**: zero-bespoke-code *is* proved by IL scan, but **4 of 9 bosses fault before phase 3**. ④ determinism baseline — **met**, `LogHash` stable across runs and across independently loaded snapshots. 🔴 **Separately, `05` §9's guardrails 1, 5 and 6 FAIL and 3, 4 are unmeasurable**, and **no build at par has ever taken a boss below 66 % HP** — every phase-2/phase-3 mechanic in the game is currently unreachable. Those are tuning decisions on `game-data/` (`21` §3.2), not code. **Merges only after M1 is merged and verified.** Next: rule on M2-R1…R4 below, then re-verify ③ |
