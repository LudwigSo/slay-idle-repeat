...
| M2 | Effect DSL & combat simulation | 1 | 🔄 **implementation complete, awaiting review** — 17/17 tasks 🔍 on `milestone/M2` (+ M2-16b ⛔ deferred to M3-07). Clean build 0 warnings/0 errors · Core **2999** · Application **555** · Contract **24** · Architecture **76/76** · 0 skipped · content validation OK · 📐 baseline **46 → 38**. ⚠️ **Exit criteria are NOT all met, and the harness is why we know.** 44 ops/23 triggers/23 conditions unit-tested ✅ · `< 5 ms` ✅ (median 2.881 ms on real content) · 8 bosses from DSL data with zero bespoke code ✅ (IL-proved) · determinism baseline ✅ — but **`05` §9's guardrails 1, 5 and 6 fail and 3, 4 are unmeasurable**, and **no boss has ever reached phase 2 at par**. Those are tuning decisions on `game-data/` (`21` §3.2), not code. **Runs in parallel with M1; merges only after M1 is merged and verified.** Next: `/milestone-review M2` |
