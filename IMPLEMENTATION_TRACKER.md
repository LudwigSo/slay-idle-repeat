...
| M1-06 | `GameRules.Apply` façade: dispatch table, `CommandResult`, `WorldSlice`, total/pure/synchronous/immutable properties; **plus the `RunRngScope` write-back choke point and its architecture rule (kickoff §5), and the single M1 gap register** | 30 §2 | 🔄 dispatched wave 6 · `feature-M1-06-game-rules-apply` |
