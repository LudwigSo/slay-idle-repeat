...
| M1 | Core domain skeleton & `InMemoryGame` | 1 | 🔄 **reviewed on `review/M1` 2026-08-13 — awaiting merge.** Both exit criteria met (one with a recorded scope caveat). Clean Release build 0/0 · unit **1964** · architecture **67/67** · content validation, vendor uniqueness and no-cloud-credentials all pass. 🔒 **`30`'s claim that the whole game is playable in memory is NOT yet true, and the next milestone must not inherit it as settled:** `30` §6's own sketch sends `StartRun → RollDice → PickPerk` and **one of forty-nine commands does anything**. §2.1's **P5** is out of scope by design; **P3 is an accepted exception** — 19 run rows throw rather than rejecting, and `START_RUN` is unreachable through any caller. **M3-15 makes the claim true, not M1** |
