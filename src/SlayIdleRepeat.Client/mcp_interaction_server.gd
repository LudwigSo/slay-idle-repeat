# VENDORED — do not edit as if it were ours, and do not upgrade it silently.
#
# Source : https://github.com/tugcantopaloglu/godot-mcp
# File   : src/scripts/mcp_interaction_server.gd
# Commit : fcbc29e03297900b9a1fcc6c7cbae116fe9d532a
# Version: 3.1.0
# Licence: MIT
#
# The runtime half of the Godot MCP server: a TCP listener on 127.0.0.1:9090 that
# accepts JSON commands from an AI agent — screenshots, node inspection, input
# simulation, and arbitrary code evaluation. It is a DEVELOPMENT TOOL. Nothing in the
# game reads it and nothing should.
#
# It is a copy rather than a reference because it carries one local change, marked
# 🔴 in _ready() below. Diff against upstream before taking a newer version.
#
# To use it, register it in project.godot as an autoload named McpInteractionServer —
# and say there, in a comment, that it is dev-only and why, the way every other entry
# in that hand-written file states its reason.
...
func _ready() -> void:
	# 🔴 THE ONE LOCAL MODIFICATION, and the reason this file is vendored rather than
	# pointed at. Upstream's _ready opens the port unconditionally, so registered as a
	# plain autoload it would listen inside the shipped Android and iOS builds — where a
	# localhost port that accepts `eval` is reachable by every other app on the handset.
	#
	# has_feature("editor") is true only when an EDITOR binary is running the project,
	# which is what Run-Game.ps1 launches and what the Godot editor's play button uses.
	# Every exported template answers false, debug and release alike, so a test APK on a
	# real device is covered as well as a store build.
	#
	# ⚠️ Re-apply this on every upgrade. Copying a newer build/scripts/ over this file
	# silently removes the gate, and nothing on this project would fail.
