...
internal sealed class HandlerInput
{
    ...
    internal MetaDrawScope MetaDraws =>
        _metaDraws ??= new MetaDrawScope(RequireMetaRegime() ?? throw new InvalidOperationException(
            ...
            "regimes is that the domain never invents entropy. 14 §2.3 marks eleven meta rows ⚄ " +
            "(BEGIN_SESSION, MERGE, ENHANCE, REROLL_QUEST, SPIN_WHEEL, REFORGE_ITEM, RETUNE_ITEM, " +
            "OPEN_CHEST, OPEN_EGG, OPEN_CRATE, START_DUEL) and every one of them must be handed a " +
            "seed; note 0 is a legitimate seed and not an absence. MERGE and ENHANCE joined that set " +
            "with M4-04, which found both drawing and neither marked — a host wired against the " +
            "older nine would fail here on exactly those two. " +
            "This is a MISWIRED COMPOSITION ROOT, not a player " +
