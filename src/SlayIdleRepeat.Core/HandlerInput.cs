...
internal sealed class HandlerInput
{
    ...
    internal RunRngScope Rng => _rng ?? throw new InvalidOperationException(
        ...
        "A scope here would give it a counter the design says it must not have. If the command draws " +
        "from the run's streams, its dispatch row is classified CommandKind.Meta and should not be.");
}
