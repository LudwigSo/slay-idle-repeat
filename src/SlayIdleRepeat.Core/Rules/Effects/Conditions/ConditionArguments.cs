...
/// <summary>
/// The two optional argument keys a condition function may carry: a status id and an optional
/// category.
/// </summary>
/// <remarks>
/// Exists as its own type, rather than the evaluator taking a <see cref="ConditionTerm"/> directly,
/// because <see cref="ValueScale"/> reuses the same condition functions and needs the same keys
/// read the same way, so a function can't answer a scale differently from a condition.
/// </remarks>
...
internal readonly record struct ConditionArguments(string? StatusId, string? Category)
{
    ...
    internal static ConditionArguments Of(ConditionTerm term)
    {
        ...
        return new ConditionArguments(term.StatusId, term.Category);
    }
    ...
    internal static ConditionArguments Of(ValueScale scale)
    {
        ...
        return new ConditionArguments(scale.StatusId, scale.Category);
    }
}
