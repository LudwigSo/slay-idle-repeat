...
/// <param name="NodeId">
/// The node's identity, and the value a run's position carries. Not interchangeable with
/// <paramref name="LinearIndex"/>: a branch node shares its index with a spine node.
/// </param>
/// <param name="Tile">The tile kind this node resolves as.</param>
/// <param name="LinearIndex">
/// The node's distance along the track in walk order. A branch node carries the index of the spine
/// node the same forward distance from its junction, so a fork costs the steps it shadows.
/// </param>
