...
/// <remarks>
/// <para>
/// <b>Implementation decisions this task's spec left open</b> (recorded here rather than in a
/// separate doc, since this is the one place they matter):
/// </para>
/// <list type="bullet">
/// <item>
/// <b>Fork label count and choice (the documented gap):</b> §3's pseudocode names two branch
/// biases ("Perilous"/"Sheltered") while §3.1's table lists four. Reading: a fork's spine
/// continuation is the "safe" path (already generated by Step 2, never separately biased) and its
/// <b>one</b> generated branch is the "risk profile" choice — so each fork draws exactly
/// <b>one</b> label, uniformly from the 4-label pool, and that label biases only the branch's own
/// weighted table. This also explains why §3.1's Sheltered row can name <see cref="TileKind.Campfire"/>
/// even though no shipped chapter's <c>tileWeights</c> includes it: the bias multiplies whatever
/// weight a kind already has in the stage's table, so a kind absent from the table stays
/// undrawable — an honest no-op, not a fabricated weight.
/// </item>
/// <item>
/// <b><see cref="DeterministicRng.Range"/> is exclusive of its upper bound</b> (`14` §8.0), while
/// §3's pseudocode writes inclusive-looking calls (<c>rng.Range(1, 2)</c> for "1-2 forks",
/// <c>rng.Range(2, min(4, …))</c> for "2-4 nodes"). Every call below adds 1 to the stated upper
/// bound so the drawn range matches the prose exactly.
/// </item>
/// <item>
/// <b>C3's "per 8 nodes" window</b> is read as non-overlapping windows of 8 from the start of each
/// stage (last window may be shorter), checked and repaired after the weighted fill — the doc
/// does not say sliding vs. non-overlapping, and non-overlapping is the reading that terminates
/// in a bounded, position-stable repair pass.
/// </item>
/// <item>
/// <b>Fork placement never overlaps another fork</b> in the same stage (shares no spine index with
/// an existing junction, and does not fall inside another fork's branch shadow or land on its
/// rejoin node) — required for the graph to stay a valid DAG; the doc's per-fork pseudocode is
/// silent on the multi-fork case.
/// </item>
/// <item>
/// C1/C2/C4/C5 are re-applied inside branch generation (branch-local sequence only), per this
/// task's brief: "redistribute proportionally, still respecting C1-C7". C3 (a density rule over an
/// 8-node window) and C6 (a run-opening rule) do not apply to a 2-4 node branch and are not
/// re-applied there. C7's global counts are tallied over the whole board, branches included, but
/// injection (if still short) only ever replaces a spine tile, never a branch tile — injecting
/// into a branch would silently make its preview dishonest after the branch already committed to
/// a label.
/// </item>
/// </list>
/// </remarks>
internal static class BoardGenerator
{
    ...
    /// exhaustion falls back to <see cref="TileKind.Enemy"/> exactly as `03` §3 states.
    /// </summary>
