...
/// <summary>
/// 🔒 `03` §1.1 — the movement engine (M3-02): stepwise traversal of a <see cref="BoardGraph"/>,
/// the junction pause, the stage-end clamp, the boss-exact rule and the Portal jump's extra
/// pre-boss campfire clamp. Pure: no <c>Run</c>, no RNG draw of its own beyond the one Portal
/// distance draw <see cref="DrawPortalDistance"/> exposes as a seam.
/// </summary>
/// <remarks>
/// <para>
/// ⚠️ <b>The virtual trailhead (`03` §1.1, position −1) is not a <see cref="NodeId"/> and is
/// deliberately outside this type's vocabulary.</b> <see cref="BoardGraph"/> has no node for it —
/// <c>Run.Position</c>'s own floor is the one place it is represented. A caller standing at the
/// trailhead consumes the first step of its roll onto <see cref="BoardGraph.FirstNodeId"/> itself,
/// then calls <see cref="Advance"/> for the remaining steps; <c>Handlers.RollDice</c> is that
/// caller.
/// </para>
/// <para>
/// 🔒 <b>Every departure from a junction is a choice, including the spine's own continuation.</b>
/// `03` §1.1: <em>"Fork choice is always free/explicit, no perk/face/item required."</em> —
...
{
    /// <summary>`03` §1.1 — a Portal jump distance is drawn uniformly from 3 to 6, inclusive.</summary>
    ...
    /// leaves the junction — always at least 1 (`03` §1.1: a junction landed on with zero movement
    /// left does not pause).
    /// </param>
    ...
    /// (and, per `03` §1.1, any chain in progress) ends here regardless of unspent steps.
    /// </param>
    ...
    /// <summary>
    /// 🔒 `03` §1.1 — steps <paramref name="steps"/> times from <paramref name="start"/>, one edge at
    /// a time, honouring the junction pause, the stage-end clamp and the boss-exact rule.
    /// </summary>
    ...
    internal static AdvanceResult Advance(BoardGraph board, NodeId start, int steps)
    {
        ...
        while (remaining > 0)
        {
            ...
            // choice (this type's remarks), so even the single-edge-looking "continue straight"
            // option is one of the two the pause offers, never taken silently.
            ...
            {
                // 🔒 Stage-end clamp (`03` §1.1), with the boss-exact rule as its one named exception:
                // "any roll taken from stage 3's last node moves exactly one step onto the boss node."
    ...
    /// <summary>
    /// 🔒 `03` §1.1 — a Portal jump: <see cref="Advance"/>'s stepwise traversal (junctions inside the
    /// jump still pause, the stage-end clamp and boss-exact rule still apply), plus stage 3's extra
    /// rule: the jump may never carry past the guaranteed pre-boss campfire — a distance that would
    /// lands ON the campfire instead. "Consequence: Portal can never reach the boss node."
    /// </summary>
    /// <param name="board">The run's board.</param>
    /// <param name="start">The Portal tile's own node. Never inside the last 4 nodes of a stage (C4).</param>
    /// <param name="distance">The drawn jump distance, `03` §1.1's 3–6 inclusive.</param>
    /// <remarks>
    /// ⚠️ <b>The clamp is applied only to a completed or boss-reaching result.</b> A jump that pauses
    /// at a junction mid-flight is left as-is: `03` §3's fork-placement rule keeps every junction
    /// candidate at a local index no later than <c>spineLength − 4</c>, strictly before the pre-boss
    /// campfire at <c>spineLength − 2</c>, so a paused Portal jump can never itself have already
    /// passed the campfire. Re-applying this clamp to whatever remains <b>after</b> a
    /// <c>CHOOSE_FORK</c> resumes a paused Portal jump is the resuming caller's concern — no handler
    /// resumes a Portal jump today (Portal tiles resolve under M3-03's <c>RESOLVE_TILE</c>, not yet
    /// built), so nothing in this milestone exercises that path; the seam is exposed here, tested in
    /// isolation, and ready for M3-03 to wire in.
    /// </remarks>
    /// <exception cref="ArgumentNullException"><paramref name="board"/> is null.</exception>
    ...
    internal static AdvanceResult AdvancePortal(BoardGraph board, NodeId start, int distance)
    {
        ...
        var campfireLinearIndex = bossLinearIndex - 2; // BoardGenerator always places it there (03 §3 Step 1).
    ...
    /// <summary>
    /// 🔒 `03` §1.1 — draws a Portal jump's distance: uniform 3–6 inclusive, from the run's own
    /// <c>board</c> stream (`14` §8.1 §14). Never player/client chosen.
    /// </summary>
