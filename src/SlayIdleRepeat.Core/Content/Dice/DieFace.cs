...
/// <summary>One face of the die: <c>{ DieFaceKind Kind; int Value; int Tier; }</c>.</summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// <see cref="Tier"/> is 0..3 and scales the face's non-movement effect only (e.g.
/// <see cref="DieFaceKind.Surge"/>'s heal, 12/15/18/21% at T0-3). There is no cosmetic tier: a kind
/// whose effect is not tied to Tier simply does not read it.
/// </para>
/// <para>
/// A validating factory rather than a public constructor: a <c>default</c> value is a distinct,
/// invalid state (<see cref="IsUnset"/>) rather than a face that silently reads as <c>Pip 0</c>.
/// </para>
/// </remarks>
public readonly record struct DieFace
{
    /// <summary>The floor for <see cref="Tier"/>.</summary>
    public const int MinTier = 0;

    /// <summary>The ceiling for <see cref="Tier"/>.</summary>
    ...
    /// <summary>Which of the six faces this is. <c>default</c> for an unset value — see <see cref="IsUnset"/>.</summary>
    ...
    /// <summary>0..3 — scales the face's non-movement effect only. No cosmetic tier.</summary>
    public int Tier { get; }
    ...
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="pips"/> is outside 1..6.</exception>
