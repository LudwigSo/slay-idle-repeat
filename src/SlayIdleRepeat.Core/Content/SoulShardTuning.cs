...
/// <summary>
/// The Soul Shard sources this milestone pays: a Boss kill (<c>BOSS_KILL_*_PER_CHAPTER</c>) and a
/// first Chapter/Tier clear, read out of <c>tuning/currencies.json#/soulShards/sources</c>.
/// </summary>
/// <remarks>
/// <para>
/// <c>_PER_CHAPTER</c> is a linear multiple of the chapter, not <see cref="ChapterScalarTuning"/>'s
/// exponential curves: the three keys are named after the chapter directly, with no growth base
/// authored alongside them, so <see cref="BossKillShards"/> reads them as
/// <c>perChapterValue(tier) * chapterId</c>.
/// </para>
/// <para>
/// The first-clear grant amount is a judgement call, not an authored number: only a min/max range
/// is authored, and <see cref="FirstClearShards"/> answers the midpoint, which this reader
/// validates falls within the authored bounds so a future data edit narrowing the range fails
/// loudly instead of silently granting outside it.
/// </para>
/// </remarks>
internal sealed class SoulShardTuning
{
    /// <summary>The document the soulShards block lives in.</summary>
    ...
    /// <summary>Normal-tier Boss-kill Soul Shards, per chapter. 15 as shipped.</summary>
    internal const string BossKillNormalReference = SourcesPointer + "/BOSS_KILL_NORMAL_PER_CHAPTER";

    /// <summary>Heroic-tier Boss-kill Soul Shards, per chapter. 40 as shipped.</summary>
    internal const string BossKillHeroicReference = SourcesPointer + "/BOSS_KILL_HEROIC_PER_CHAPTER";

    /// <summary>Mythic-tier Boss-kill Soul Shards, per chapter. 100 as shipped.</summary>
    internal const string BossKillMythicReference = SourcesPointer + "/BOSS_KILL_MYTHIC_PER_CHAPTER";

    /// <summary>The floor of the first-clear grant range. 100 as shipped.</summary>
    internal const string FirstClearMinReference = SourcesPointer + "/FIRST_CLEAR_MIN";

    /// <summary>The ceiling of the first-clear grant range. 800 as shipped.</summary>
    ...
    /// (100..800). See this type's remarks.
    /// </summary>
    ...
    /// <summary>The Soul Shards a Boss kill pays at <paramref name="chapterId"/> and <paramref name="tier"/>.</summary>
