...
/// <summary>
/// <c>TILE_CACHE</c>'s Beast Feed payout and its Pet Egg rate, read out of
/// <c>tuning/currencies.json#/inRunIncome/cache</c>.
/// </summary>
/// <remarks>
/// The Feed payout scales by <see cref="ChapterScalarTuning"/>'s <c>M(c)</c> curve; the egg rate
/// is deliberately not scaled — it is flat across chapters.
/// </remarks>
internal sealed class CacheTuning
{
    /// <summary>The document the cache block lives in.</summary>
    ...
    /// <summary>The chapter-1 Beast Feed payout. 25 as shipped.</summary>
    internal const string BeastFeedBaseReference = CachePointer + "/beastFeedBase";

    /// <summary>The flat Pet Egg rate. 0.06 as shipped.</summary>
    ...
    /// <summary>The Beast Feed payout before <c>M(c)</c>.</summary>
    internal long BeastFeedBase { get; }

    /// <summary>
    /// The probability a cache pays a Pet Egg instead of Feed, in <c>[0,1]</c>. Flat across
    /// chapters.
    /// </summary>
