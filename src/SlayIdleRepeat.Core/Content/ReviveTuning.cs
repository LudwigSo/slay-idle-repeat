...
/// <summary>
/// 🔒 M3-13, `02` §6 — the once-per-run <c>AD_REVIVE</c> placement's numbers, read out of
/// <c>tuning/ads.json#/placementRewardValues/AD_REVIVE</c>.
/// </summary>
internal sealed class ReviveTuning
{
    /// <summary>The document `12` §4's placement values live in.</summary>
    internal const string DocumentPath = "tuning/ads.json";

    /// <summary>`02` §6 / `12` §4.3 — the placement id, and the <c>Run.AdUses</c> key `02` §6's once-per-run gate reads.</summary>
    ...
    /// <summary>`02` §6 — the share of Max HP a revive restores. 0.5 as shipped.</summary>
    internal const string HealPctMaxHpReference = ValuesPointer + "/healPctMaxHp";

    /// <summary>`02` §6 — the invulnerability window a revive grants, in seconds. 2 as shipped.</summary>
    ...
    /// <summary>`02` §6 — the share of Max HP a revive restores. 0.5 (50%) as shipped.</summary>
    internal double HealPctMaxHp { get; }

    /// <summary>
    /// 🔒 `02` §6 — the invulnerability window a revive grants, in seconds. Recorded here so the fact
    /// is not silently dropped; nothing in <c>Core</c> simulates combat time yet (M4's), so no rule
    /// consumes it today — see <c>Handlers.Revive</c>'s remarks for what that means for this task.
    /// </summary>
